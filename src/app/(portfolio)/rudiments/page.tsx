      ...
      <div className="space-y-8 mt-16">
        {rudimentsByCategory.map((category: RudimentGroup, index) => (
          <div className="space-y-4" key={index}>
            <h2 className="text-xl md:text-2xl font-semibold tracking-tight text-zinc-800 border-b border-mzinc-700/40 pb-2">
              {category.title}
            </h2>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {category.items.map((item: Rudiment) => (
                <div className="bg-zinc-900 backdrop-blur-sm rounded-lg p-4 transition-all group cursor-pointer z-0 hover:scale-105 bg-opacity-0 transition hover:opacity-100 hover:bg-zinc-800/50" key={item._id}>

                  <h3 className="font-medium text-zinc-100 group-hover:text-orange-500 transition-colors mb-2">
                    {item.name}
                  </h3>
